...
RSpec.describe "Articles", type: :request do
  fdescribe "GET /api/v1/articles" do
    ...
    let!(:article4) { create(:article, updated_at: 1.days.ago,) }
    let!(:article5) { create(:article, updated_at: 2.days.ago) }
    let!(:article6) { create(:article) }
  ...
  describe "GET /api/v1/articles/:id" do
    ...
    context "指定したidの記事が存在する場合" do
      ...
      context "記事のstatusがpublishedだった時" do
      ...
      context "記事のstatusがdraftだった時" do
        let(:article) { create(:article,:draft) }

        it "記事の詳細の取得に失敗する" do
          expect { subject }.to raise_error ActionController::MissingExactTemplate
        end
      end
    end
